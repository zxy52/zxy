...
def 上下插销孔(inputs, file_path='C:/Users/朱许杨/Desktop/毕业项目/上下插销孔.xlsx', epochs=10000):
    # 读取数据集
    ...
    model = tf.keras.Sequential([
        tf.keras.layers.SimpleRNN(128, activation='relu', input_shape=(None, len(features))),
    ...
    # 训练模型
    model.fit(X_train_np, y_train_np, epochs=epochs, verbose=0)
    ...
    # 重塑输入数据为 RNN 所需的形状
    input_data_np = input_data_np.reshape((input_data_np.shape[0], 1, input_data_np.shape[1]))
...
def 中控插销孔(inputs, file_path='C:/Users/朱许杨/Desktop/毕业项目/中控插销孔.xlsx', epochs=10000):
    # 读取数据集
    data = pd.read_excel(file_path)
    ...
    return A_result, B_result
def 主锁孔(inputs, file_path='C:/Users/朱许杨/Desktop/毕业项目/主锁孔.xlsx', epochs=10000):
    # 读取数据集
    data = pd.read_excel(file_path)

    # 特征编码
    label_encoders = {}
    for column in ['子/母', '前板/后板', '孔型', '锁型']:
        label_encoders[column] = LabelEncoder()
        data[column] = label_encoders[column].fit_transform(data[column])
    ...
    model = tf.keras.Sequential([
        tf.keras.layers.SimpleRNN(128, activation='relu', input_shape=(None, len(X.columns))),
    ...
    # 构建输入数据的 NumPy 数组
    input_data_np = np.array([[thickness, label_encoders["子/母"].transform([type_input])[0],
                                   label_encoders["前板/后板"].transform([plate_input])[0],
                                   label_encoders["孔型"].transform([hole_type])[0],
                                   label_encoders["锁型"].transform([lock_type])[0]]], dtype=np.float32)
    ...
    def calculate_B(type_input, plate_input, predicted_values):
        if type_input == 0 and plate_input == 0:
            B = predicted_values[1]  # 假设条件，可以根据实际逻辑修改
        else:
            B = predicted_values[1]
        return B

    A_result = calculate_A(predicted_values)
    B_result = calculate_B(label_encoders["子/母"].transform([type_input])[0],
                           label_encoders["前板/后板"].transform([plate_input])[0],
                           predicted_values)

    return A_result, B_result
def 副锁孔(inputs, file_path='C:/Users/朱许杨/Desktop/毕业项目/副锁孔.xlsx', epochs=10000):
    # 读取数据集
    data = pd.read_excel(file_path)
    ...
    B1_result = calculate_B1(predicted_values, 主锁孔B)
    ...
    B2_result = calculate_B2(predicted_values, 主锁孔B)

    return A_result, B1_result, B2_result
def 拉手孔(inputs, file_path='C:/Users/朱许杨/Desktop/毕业项目/拉手孔.xlsx', epochs=10000):
    try:
        # 读取数据集
        data = pd.read_excel(file_path)
        ...
        model = tf.keras.Sequential([
            tf.keras.layers.SimpleRNN(128, activation='relu', input_shape=(1, len(features))),
            tf.keras.layers.Dense(len(targets))
        ])
        ...
        return None, None
def 暗铰链孔(inputs, file_path='C:/Users/朱许杨/Desktop/毕业项目/暗铰链孔.xlsx', epochs=10000):
    # 读取数据集
    data = pd.read_excel(file_path)
...
def 明铰链孔(inputs, file_path='C:/Users/朱许杨/Desktop/毕业项目/明铰链孔.xlsx', epochs=10000):
    # 读取数据集
    data = pd.read_excel(file_path)
    ...
    return A_result, B_result, C_result, D_result, E_result
def 硬标(inputs, file_path='C:/Users/朱许杨/Desktop/毕业项目/硬标.xlsx', epochs=10000):
    # 读取数据集
    data = pd.read_excel(file_path)
    ...
    return A_result, B_result
def 猫眼孔(inputs, file_path='C:/Users/朱许杨/Desktop/毕业项目/猫眼孔.xlsx', epochs=10000):
    # 读取数据集
    data = pd.read_excel(file_path)
    ...
    return A_result, B_result
def 商标(inputs, file_path='C:/Users/朱许杨/Desktop/毕业项目/商标.xlsx', epochs=10000):
    # 读取数据集
    data = pd.read_excel(file_path)
    ...
    return A_result, B_result
def 压纹(inputs, file_path='C:/Users/朱许杨/Desktop/毕业项目/压纹.xlsx', epochs=10000):
    # 读取数据集
    data = pd.read_excel(file_path)
    ...
    return A_result, B_result
def 挂钩孔(inputs, file_path='C:/Users/朱许杨/Desktop/毕业项目/挂钩孔.xlsx', epochs=10000):
    # 读取数据集
    data = pd.read_excel(file_path)
...
def 折弯外形(inputs, file_path='C:/Users/朱许杨/Desktop/毕业项目/折弯外形.xlsx', epochs=10000):
    # 读取数据集
    data = pd.read_excel(file_path)
    ...
    return A_result, B_result
def 折弯偏花(inputs, file_path='C:/Users/朱许杨/Desktop/毕业项目/折弯偏花.xlsx', epochs=10000):
    # 读取数据集
    data = pd.read_excel(file_path)
